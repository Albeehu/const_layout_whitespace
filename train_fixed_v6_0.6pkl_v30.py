"""用 train_fixed_v6_0.6pkl_v29 改 v30
...
def main():
    ...
    for epoch in range(10000):
        for data in train_dataloader:
            ...
            # 2. [升級：元件間一致性對齊]
            # 取得所有有效元件的左邊界 x1
            x1_all = (bbox_fake[:, :, 0] - bbox_fake[:, :, 2] / 2).clamp(0, 1)
            # 這裡計算 batch 內元件左邊界的「集中度」，強迫多個元件對齊成一條垂直線
            if mask.any():
                loss_align_consist = x1_all[mask].std() 
                loss_G += (loss_align_consist * 60.0) 

            # 3. [升級：排版階層感]
            img_mask = (label == IMG_ID) & valid_mask
            if img_mask.any():
                # A. 強迫圖片（紫色）的中心點偏向上半部，建立視覺重心
                img_centers_y = bbox_fake[img_mask][:, 1]
                loss_img_pos = (img_centers_y - 0.3).pow(2).mean() 
                loss_G += (loss_img_pos * 50.0)
                
                # B. 強迫生成的圖片面積至少要達到 10% (0.1)
                img_areas = bbox_fake[img_mask][:, 2] * bbox_fake[img_mask][:, 3]
                loss_img_min_area = torch.relu(0.10 - img_areas).mean()
                loss_G += (loss_img_min_area * 150.0)

            # 4. [格線對齊輔助]
            loss_grid = torch.min(torch.abs(x1_all - 0.1), torch.abs(x1_all - 0.5)).mean()
            loss_G += (loss_grid * 40.0)
