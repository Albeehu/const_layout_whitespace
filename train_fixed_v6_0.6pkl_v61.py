"""用 train_fixed_v6_0.6pkl_v60 改 v61
1. top style: top_margin > bottom_margin
"""
import os
import csv
import argparse
...
def _margin_stats_from_valid_boxes(boxes_xyxy: torch.Tensor):
    """Return L,R,T,B margins and union stats for valid xyxy boxes."""
    x1 = boxes_xyxy[:, 0].min()
    y1 = boxes_xyxy[:, 1].min()
    x2 = boxes_xyxy[:, 2].max()
    y2 = boxes_xyxy[:, 3].max()
...
def frame_balance_loss(
    bbox_fake: torch.Tensor,
    label: torch.Tensor,
    padding_layout: torch.Tensor,
    lambda_frame: float = 1.0,
    min_margin: float = 0.16,
    target_union_w: float = 0.28,
    target_union_h: float = 0.42,
    center_y: float = 0.50,
):
    """Frame-only balance loss for true four-side whitespace.

    This loss is ONLY intended for the frame_vertical template.
    It does not alter the common whitespace_style_loss used by other templates.
    """
    device = bbox_fake.device
    losses = []
    ...
    for b in range(bbox_fake.size(0)):
        ...
        union_w = (x2 - x1).clamp(min=0.0)
        union_h = (y2 - y1).clamp(min=0.0)
        ccx = (x1 + x2) / 2.0
        ccy = (y1 + y2) / 2.0

        margins = torch.stack([L, R, T, B])
        loss_b = torch.tensor(0.0, device=device)

        # all four sides should have visible whitespace
        loss_b += torch.relu(min_margin - L)
        loss_b += torch.relu(min_margin - R)
        loss_b += torch.relu(min_margin - T)
        loss_b += torch.relu(min_margin - B)

        # balance four margins
        loss_b += 2.5 * ((L - R) ** 2 + (T - B) ** 2)
        loss_b += 1.5 * margins.var(unbiased=False)

        # centered compact block, but NOT a tall narrow pillar
        loss_b += 2.0 * ((ccx - 0.50) ** 2)
        loss_b += 1.6 * ((ccy - center_y) ** 2)
        loss_b += 2.0 * torch.relu(union_w - target_union_w)
        loss_b += 2.0 * torch.relu(union_h - target_union_h)

        # avoid collapsing to an overly tiny block
        loss_b += 0.8 * torch.relu(0.16 - union_w)
        loss_b += 0.6 * torch.relu(0.20 - union_h)

        losses.append(loss_b)

    if not losses:
        return torch.tensor(0.0, device=device)
    return lambda_frame * torch.stack(losses).mean()


def frame_outer_band_loss(
    bbox_fake: torch.Tensor,
    label: torch.Tensor,
    padding_layout: torch.Tensor,
    lambda_outer: float = 1.0,
    band_margin: float = 0.16,
    soft_margin: float = 0.00,
    top_weight: float = 1.35,
    target_ids=(SVG_ID, TEXT_ID, IMG_ID),
):
    """Frame-only per-element outer-band penalty.

    Each valid content element is penalized if any part of its box enters the
    forbidden outer ring near the canvas boundary. This is intentionally
    stronger than union-box losses, because tiny top elements often do not move
    the union box much but still visibly break the four-side whitespace style.
    """
    device = bbox_fake.device
    losses = []
    ...
    for b in range(bbox_fake.size(0)):
        ...
        left_intrude = torch.relu((band_margin + soft_margin) - x1)
        right_intrude = torch.relu(x2 - (1.0 - band_margin - soft_margin))
        top_intrude = torch.relu((band_margin + soft_margin) - y1)
        bottom_intrude = torch.relu(y2 - (1.0 - band_margin - soft_margin))

        loss_elem = left_intrude + right_intrude + top_weight * top_intrude + bottom_intrude
        losses.append(loss_elem.mean())

    if not losses:
        return torch.tensor(0.0, device=device)
    return lambda_outer * torch.stack(losses).mean()


def frame_shape_loss(
    bbox_fake: torch.Tensor,
    label: torch.Tensor,
    padding_layout: torch.Tensor,
    lambda_shape: float = 1.0,
    target_union_w: float = 0.30,
    target_union_h: float = 0.34,
    max_ar: float = 1.30,
    min_w: float = 0.18,
    min_h: float = 0.18,
    center_weight: float = 1.0,
):
    """Frame-only union-shape penalty.

    Encourages a compact central rectangle instead of a tall narrow pillar.
    Aspect ratio is defined as union_h / union_w.
    """
    device = bbox_fake.device
    losses = []
    ...
    for b in range(bbox_fake.size(0)):
        ...
        union_w = (x2 - x1).clamp(min=1e-6)
        union_h = (y2 - y1).clamp(min=1e-6)
        ccx = (x1 + x2) / 2.0
        ccy = (y1 + y2) / 2.0
        ar = union_h / union_w

        loss_b = torch.tensor(0.0, device=device)
        loss_b += center_weight * ((ccx - 0.50) ** 2 + (ccy - 0.50) ** 2)
        loss_b += 1.8 * torch.relu(union_w - target_union_w)
        loss_b += 1.8 * torch.relu(union_h - target_union_h)
        loss_b += 0.8 * torch.relu(min_w - union_w)
        loss_b += 0.8 * torch.relu(min_h - union_h)
        loss_b += 2.2 * torch.relu(ar - max_ar)

        losses.append(loss_b)

    if not losses:
        return torch.tensor(0.0, device=device)
    return lambda_shape * torch.stack(losses).mean()
...
def poster_template_loss(
    ...
    for b in range(bbox_fake.size(0)):
        ...
        if template == "center_vertical":
        ...
        elif template == "frame_vertical":
            left_margin = x1_all.min()
            right_margin = 1.0 - x2_all.max()
            top_margin = y1_all.min()
            bottom_margin = 1.0 - y2_all.max()
            union_w = x2_all.max() - x1_all.min()
            union_h = y2_all.max() - y1_all.min()
            union_cx = 0.5 * (x1_all.min() + x2_all.max())
            union_cy = 0.5 * (y1_all.min() + y2_all.max())

            # True four-side whitespace: centered compact content block
            loss_b += ((union_cx - 0.50) ** 2) * 4.0
            loss_b += ((union_cy - 0.50) ** 2) * 3.0
            loss_b += torch.relu(union_w - 0.30) * 3.5
            loss_b += torch.relu(union_h - 0.46) * 3.5

            # balanced margins on all four sides
            loss_b += torch.relu(0.16 - left_margin) * 4.0
            loss_b += torch.relu(0.16 - right_margin) * 4.0
            loss_b += torch.relu(0.16 - top_margin) * 4.0
            loss_b += torch.relu(0.16 - bottom_margin) * 4.0
            loss_b += ((left_margin - right_margin) ** 2) * 3.0
            loss_b += ((top_margin - bottom_margin) ** 2) * 3.0

            # Keep only gentle centering constraints; do NOT force text-top / image-bottom
            if text_m.any():
                t = boxes[text_m]
                loss_b += ((t[:, 0] - 0.50) ** 2).mean() * 1.5

            if graphic_m.any():
                g = boxes[graphic_m]
                loss_b += ((g[:, 0] - 0.50) ** 2).mean() * 1.2
            ...
            # TOP whitespace version: keep the whole content block in the lower half.
            # Previous version allowed text around y=0.14, which destroyed the top margin.
            loss_b += ((union_cx - 0.50) ** 2) * 2.5
            loss_b += ((union_cy - 0.64) ** 2) * 4.5
            loss_b += torch.relu(union_w - 0.56) * 2.5
            loss_b += torch.relu(union_h - 0.52) * 3.0

            # Key signal for top whitespace: top margin must be clearly larger than bottom margin.
            loss_b += torch.relu(0.24 - top_margin) * 5.0
            loss_b += torch.relu((bottom_margin + 0.18) - top_margin) * 6.0

            # keep side margins reasonable and avoid drifting to one side
            loss_b += torch.relu(0.06 - left_margin) * 1.5
            loss_b += torch.relu(0.06 - right_margin) * 1.5
            loss_b += ((left_margin - right_margin) ** 2) * 1.2
            ...
            if img_m.any():
                ...
                loss_b += ((main_img[1] - 0.66) ** 2) * 2.5
                loss_b += torch.relu(0.16 - main_area) * 3.5

            if text_m.any():
                # Text should not enter the upper blank area.
                loss_b += torch.relu(0.40 - boxes[text_m][:, 1]).mean() * 3.0
                loss_b += torch.relu(boxes[text_m][:, 1] - 0.86).mean() * 1.5
            ...
            # bottom whitespace: keep the whole content block in the upper half
            loss_b += ((union_cx - 0.50) ** 2) * 3.0
            loss_b += ((union_cy - 0.40) ** 2) * 4.0
            loss_b += torch.relu(union_w - 0.42) * 3.0
            loss_b += torch.relu(union_h - 0.42) * 3.0

            # the key signal: require a clearly visible bottom margin
            loss_b += torch.relu(0.20 - bottom_margin) * 5.0

            # top can be smaller than bottom, but should not collapse to zero
            loss_b += torch.relu(0.04 - top_margin) * 1.5

            # keep side margins reasonable and avoid extreme horizontal drift
            loss_b += torch.relu(0.06 - left_margin) * 2.0
            loss_b += torch.relu(0.06 - right_margin) * 2.0
            loss_b += ((left_margin - right_margin) ** 2) * 1.5

            # main image should sit in the upper-middle area, not at the bottom
            if img_m.any():
                img_boxes = boxes[img_m]
                img_areas = img_boxes[:, 2] * img_boxes[:, 3]
                k = torch.argmax(img_areas)
                ...
                loss_b += ((main_img[0] - 0.50) ** 2) * 1.0
                loss_b += ((main_img[1] - 0.40) ** 2) * 2.5
                loss_b += torch.relu(0.18 - main_area) * 4.0

            # text should stay above the lower region so the caption block does not fall downward
            if text_m.any():
                loss_b += torch.relu(boxes[text_m][:, 1] - 0.62).mean() * 2.5
...
def frame_vertical_role_loss(
    bbox_fake: torch.Tensor,
    label: torch.Tensor,
    padding_layout: torch.Tensor,
):
    """Role-aware loss for true four-side whitespace.

    This version keeps the whole content block centered and margin-balanced,
    without forcing a top-text / bottom-image poster structure.
    """
    device = bbox_fake.device
    losses = []
    ...
    for b in range(bbox_fake.size(0)):
        ...
        loss_b = torch.tensor(0.0, device=device)
        loss_b += ((ccx - 0.50) ** 2) * 4.0
        loss_b += ((ccy - 0.50) ** 2) * 3.0

        loss_b += torch.relu(cw - 0.30) * 3.0
        loss_b += torch.relu(ch - 0.44) * 3.0

        loss_b += torch.relu(0.16 - L) * 3.5
        loss_b += torch.relu(0.16 - R) * 3.5
        loss_b += torch.relu(0.16 - T) * 3.5
        loss_b += torch.relu(0.16 - Bm) * 3.5
...
def _get_dynamic_region_cfg(template: str):
    ...
    cfg = {
        ...
        "frame_vertical": {
            "cx": 0.50, "cy": 0.50,
            "base_w": 0.10, "base_h": 0.10,
            "count_w": 0.022, "count_h": 0.022,
            "area_w": 0.22, "area_h": 0.22,
            "min_w": 0.16, "max_w": 0.32,
            "min_h": 0.16, "max_h": 0.40,
            "dir_w": 0.0, "dir_h": 0.0,
            "dir_margin": 0.00,
        },
        "right_column": {
        ...
        "bottom_caption": {
            "cx": 0.50, "cy": 0.40,
            "base_w": 0.18, "base_h": 0.16,
            "count_w": 0.055, "count_h": 0.055,
            "area_w": 0.62, "area_h": 0.56,
            "min_w": 0.24, "max_w": 0.56,
            "min_h": 0.20, "max_h": 0.50,
            "dir_w": 0.0, "dir_h": -1.0,
            "dir_margin": 0.14,
        },
        "hybrid": {
...
def top_bottom_margin_gap_loss(
    bbox_fake: torch.Tensor,
    label: torch.Tensor,
    padding_layout: torch.Tensor,
    direction: str = "top",
    min_gap: float = 0.18,
    min_large_margin: float = 0.24,
    max_small_margin: float = 0.20,
    center_x_weight: float = 0.8,
    compact_weight: float = 0.6,
    lambda_gap: float = 1.0,
):
    """Explicit top/bottom whitespace direction loss.

    direction="top": top margin should be larger than bottom margin.
    direction="bottom": bottom margin should be larger than top margin.
    This fixes the generic S_tb issue: S_tb rewards vertical difference,
    but does not strongly control which side owns the whitespace.
    """
    device = bbox_fake.device
    losses = []
    ...
    for b in range(bbox_fake.size(0)):
        ...
        if direction == "top":
            ...
            raise ValueError(f"unknown tb direction: {direction}")

        loss_b += torch.relu(min_large_margin - large) * 2.5
        loss_b += torch.relu((small + min_gap) - large) * 4.0
        loss_b += torch.relu(small - max_small_margin) * 0.8

        loss_b += center_x_weight * ((ccx - 0.50) ** 2)
        loss_b += compact_weight * torch.relu(union_w - 0.60)
        loss_b += compact_weight * torch.relu(union_h - 0.58)
        loss_b += 0.5 * ((L - R) ** 2)
    ...
    if not losses:
        return torch.tensor(0.0, device=device)
    return lambda_gap * torch.stack(losses).mean()
...
def main():
    ...
    parser.add_argument('--layout_template', type=str, default='center_vertical',
                        choices=['top_hero', 'center_vertical', 'frame_vertical', 'right_column', 'bottom_caption', 'hybrid'])
    ...
    parser.add_argument('--lambda_frame_balance', type=float, default=0.0)
    parser.add_argument('--lambda_outer_band', type=float, default=0.0)
    parser.add_argument('--frame_band_margin', type=float, default=0.16)
    parser.add_argument('--frame_band_soft', type=float, default=0.00)
    parser.add_argument('--lambda_frame_shape', type=float, default=0.0)
    parser.add_argument('--frame_target_w', type=float, default=0.30)
    parser.add_argument('--frame_target_h', type=float, default=0.34)
    parser.add_argument('--frame_max_ar', type=float, default=1.30)
    parser.add_argument('--lambda_region', type=float, default=12.0)
    parser.add_argument('--lambda_tb_gap', type=float, default=0.0,
                        help='Top/bottom directional margin-gap loss weight. Use with top_hero/bottom_caption.')
    parser.add_argument('--tb_gap_start', type=int, default=0)
    parser.add_argument('--tb_gap_warmup', type=int, default=1500)
    parser.add_argument('--tb_min_gap', type=float, default=0.18)
    parser.add_argument('--tb_min_large_margin', type=float, default=0.24)
    parser.add_argument('--tb_max_small_margin', type=float, default=0.20)
    parser.add_argument('--tb_direction', type=str, default='auto', choices=['auto', 'top', 'bottom'],
                        help='auto: top_hero=>top whitespace, bottom_caption=>bottom whitespace.')
        ...
        for data in train_dataloader:
            ...
            if curr_lambda_align > 0:
                if args.layout_template == 'center_vertical':
                    ...
                    ew, cw = 0.5, 2.0
                elif args.layout_template == 'frame_vertical':
                    gx = [0.38, 0.50, 0.62]
                    gy = [0.38, 0.50, 0.62]
                    ew, cw = 0.22, 0.45
            ...
            curr_lambda_tb_gap = get_curr_lambda(
                args.lambda_tb_gap * stage_mult['template'], iteration, args.tb_gap_start, args.tb_gap_warmup
            )
            ...
            loss_frame_balance = torch.tensor(0.0, device=device)
            loss_outer_band = torch.tensor(0.0, device=device)
            loss_frame_shape = torch.tensor(0.0, device=device)
            loss_region = torch.tensor(0.0, device=device)
            loss_tb_gap = torch.tensor(0.0, device=device)
            ...
            elif args.layout_template == 'frame_vertical' and curr_lambda_role_layout > 0:
                loss_role_layout = frame_vertical_role_loss(
                    bbox_fake,
                    label,
                    padding_layout,
                )
            ...
            if args.layout_template == 'frame_vertical' and args.lambda_frame_balance > 0:
                loss_frame_balance = frame_balance_loss(
                    bbox_fake,
                    label,
                    padding_layout,
                    lambda_frame=args.lambda_frame_balance,
                    min_margin=args.frame_band_margin,
                    target_union_w=args.frame_target_w,
                    target_union_h=max(args.frame_target_h + 0.08, args.frame_target_h),
                    center_y=0.50,
                )
                loss_G += loss_frame_balance

            if args.layout_template == 'frame_vertical' and args.lambda_outer_band > 0:
                loss_outer_band = frame_outer_band_loss(
                    bbox_fake,
                    label,
                    padding_layout,
                    lambda_outer=args.lambda_outer_band,
                    band_margin=args.frame_band_margin,
                    soft_margin=args.frame_band_soft,
                )
                loss_G += loss_outer_band

            if args.layout_template == 'frame_vertical' and args.lambda_frame_shape > 0:
                loss_frame_shape = frame_shape_loss(
                    bbox_fake,
                    label,
                    padding_layout,
                    lambda_shape=args.lambda_frame_shape,
                    target_union_w=args.frame_target_w,
                    target_union_h=args.frame_target_h,
                    max_ar=args.frame_max_ar,
                )
                loss_G += loss_frame_shape
            ...
            if curr_lambda_tb_gap > 0 and args.layout_template in ('top_hero', 'bottom_caption'):
                if args.tb_direction == 'auto':
                    tb_dir = 'top' if args.layout_template == 'top_hero' else 'bottom'
                else:
                    tb_dir = args.tb_direction
                loss_tb_gap = top_bottom_margin_gap_loss(
                    bbox_fake,
                    label,
                    padding_layout,
                    direction=tb_dir,
                    min_gap=args.tb_min_gap,
                    min_large_margin=args.tb_min_large_margin,
                    max_small_margin=args.tb_max_small_margin,
                    lambda_gap=curr_lambda_tb_gap,
                )
                loss_G += loss_tb_gap
            ...
            if iteration % 100 == 0:
                ...
                print(f'[{iteration}] Loss_D: {loss_D.item():.4f} Loss_G: {loss_G.item():.4f} '
                      f'STYLE:{curr_lambda_style:.3f} SPACE:{curr_lambda_space:.3f} FRAME:{loss_frame_balance.item():.3f} OBAND:{loss_outer_band.item():.3f} FSHAPE:{loss_frame_shape.item():.3f} REGION:{loss_region.item():.3f} TBGAP:{loss_tb_gap.item():.3f} FG:{loss_fg_area.item():.3f} TXTW:{(curr_lambda_text_width * loss_text_width).item():.3f} '
