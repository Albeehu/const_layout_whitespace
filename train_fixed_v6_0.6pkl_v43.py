"""用 train_fixed_v6_0.6pkl_v42 改 v43
    1. max_nodes 只用 priority 選 不用面積大小 且沒有被選到的image如果有face 也不該被選入
    2. image 和 image 重疊要有懲罰 但是如果image > layout 90% 可以重疊
    3. 印出face
    4. 用 fixed_sample_v47.pt
"""
import os
import csv
import argparse
...
class RawLayoutDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
            ...
            n = len(o_label)
            keep_o_idxs = np.arange(n, dtype=np.int64)  # o_label_all/o_bbox_all 中被保留的索引
            if n > self.max_nodes:
                # 只依 priority 做截斷：Image > Text > SVG > BG > Mask
                # 不再用面積排序（避免「大物件永遠被選到」）
                # 同 priority (= 同 label) 時隨機挑選保留哪些元素（只影響「保留誰」，不打亂原始順序）
                idxs = np.arange(n)
                ...
                for p in np.unique(priority):
                    ...
                    if cand.size == 0:
                        continue
                    np.random.shuffle(cand)  # 同 label & priority：隨機挑
            ...
            # 3. Face 只保留「屬於被保留 Image」的人臉
            # 你的需求：生成的 face 應該被 image 包著，
            # 因此若某張臉對應到的 image 沒被選進 max_nodes，那張臉也不該被加入 token 序列。
            ...
            if len(f_label) > 0 and len(o_label_all) > 0:
                ...
                if img_all_idx.size == 0:
                    # 沒有 image 就不保留任何 face（因為無法被 image 包含）
                    ...
                    # 計算每張臉被每張 image 覆蓋的比例（inter / face_area）
                    ...
                    # 用和 containment_loss 一致的門檻：>= 0.98 視為「被某張 image 包含」
                    contain_thr = 0.98
                    assigned_img_idx = np.where(best_ratio >= contain_thr, img_all_idx[best_m], -1)

                    # 只有「assigned_img_idx 在 keep_o_idxs」的 face 才保留
                    kept_set = set(keep_o_idxs.tolist())
                    f_keep = np.array([(ai in kept_set) for ai in assigned_img_idx], dtype=bool)
