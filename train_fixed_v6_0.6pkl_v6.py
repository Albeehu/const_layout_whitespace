"""用train_fixed_v6_0.6pkl_v4 整合了 人臉重建損失 (Face Reconstruction Loss)、
座標範圍限制 (BBox Clamp) 以及 Overlap 權重熱身機制 (Warm-up)，能有效防止人臉消失並提升佈局穩定性。"""
import os
import argparse
import pickle
...
def pairwise_overlap_loss(bbox_fake, label, padding_mask, eps=1e-5):
    ...
    # 1. 將所有類別的基礎懲罰從 1.0 提高到 2.0，強制拉開物件
    W = torch.ones((10, 10), device=device) * 2.0
    
    # 2. 針對重疊嚴重的物件 (假設 ID 0: SVG, ID 1: Text) 提高懲罰至 5.0
    W[0, :] = 5.0; W[:, 0] = 5.0
    W[1, :] = 5.0; W[:, 1] = 5.0
    
    # 3. 背景與框架類別 (假設 ID 2, 3) 允許被覆蓋，設為 0
    W[3, :], W[:, 3] = 0.0, 0.0
    W[2, :], W[:, 2] = 0.0, 0.0
    
    # 4. 針對人臉 (ID 5) 與其他物件的特殊關係
    # 確保人臉 (5) 與文字/圖片 (1) 之間有極高懲罰 (5.0)
    W[1, 5], W[5, 1] = 5.0, 5.0
...
def main():
    ...
    for epoch in range(int(1e+5)):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            # [修正 2] Overlap 熱身機制：前 5000 步權重漸進增加
            ...
            loss_G = loss_G_adv + args.lambda_ws * loss_ws + curr_lambda_ov * loss_ov + loss_face

            # [修正 3] 強制人臉重建損失：防止 Face ID 消失 (針對高品質子集強化)
            face_mask_g = (label == FACE_ID) & (~padding_mask)
            if face_mask_g.any():
                loss_face_recon_g = F.mse_loss(bbox_fake[face_mask_g], bbox_real[face_mask_g])
                loss_G += 5.0 * loss_face_recon_g  # 給予高權重確保人臉存在
            ...
            # [修正 4] 視覺化邏輯：不覆蓋 Face 座標，改為純粹生成觀察
            if iteration % args.vis_every == 0:
                ...
                with torch.no_grad():
                    ...
                    # 顯示所有物件(含Face)，排除背景(HIDE_IDS)
                    vis_mask = fixed_mask & (~hide_mask)
