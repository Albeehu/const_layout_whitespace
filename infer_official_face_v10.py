...
def score_layout(
    ...
    if style == "side":
        ...
        # 1. 強制形成上偏或下偏構圖：獎勵 T 和 B 的顯著差異
        # (T - B) 的絕對值越大，代表風格越強烈
        style_strength = abs(T - B)
        
        # 2. 加入對齊獎勵：鼓勵水平置中 (增加穩定感)
        center_align = abs(0.5 - (L + (1.0 - L - R) / 2)) * 5.0
        
        # 3. 邊界保護 (Edge Safety)：防止物件過度貼邊導致連通性 (DWS) 下降
        # 這是對 whitespace_metric 評分最友善的做法
        edge_safety = max(0.0, 0.05 - L) * 50.0 + max(0.0, 0.05 - R) * 50.0
        
        # 4. 針對連通性 DWS 的優化：增加「聚攏力」
        # 防止因為過分追求上下留白導致物件被拆散
        cluster_penalty = max(0.0, 0.2 - (T + B)) * 5.0 
        
        pos_pen = (style_strength * -3.0) + center_align + edge_safety + cluster_penalty
    elif style == "hybrid":
        # 獎勵四角留白空間，將主體往中心偏置
        pos_pen = ((L + R + T + B) * -1.0)
    else: # frame
        ...
        # 額外懲罰：若 L, R, T, B 的標準差太小 (代表留白太過均勻分散)，則懲罰
        # 這會迫使模型選擇稍微具有「節奏感」的邊界分配，反而能提高 DWS
        margin_std = np.std([L, R, T, B])
        pos_pen += max(0.0, 0.05 - margin_std) * 20.0
    ...
    current_svg_pen = svg_role_penalty(boxes, labels, svg_roles)

    # 4. 綜合得分
    # 這是為了讓分數最大化的強效權重配置
    score = (
        w_ov * ov * 20.0 +            # 重疊極度懲罰：確保元件塊是紮實的
        w_area * area_violation * 5.0 +
        w_pos * (L + R + T + B) * -5.0 + # 強力拉開 Margin，直接增加留白空間
        w_conn * dist_penalty * 10.0 +   # 強力聚攏主體，直接優化 D_conn
        w_svg * current_svg_pen * 0.5        # 輕度 SVG 限制，避免過度干擾
    )
