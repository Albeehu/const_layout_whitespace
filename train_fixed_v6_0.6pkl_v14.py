"""用 train_fixed_v6_0.6pkl_v13 改 v14
    1.  將svg element (id =0) 改成大於 15% 面積的 SVG → 改為 Image (ID 2)
        小於 15% 面積的 SVG → 保留為 SVG
"""
import os
import argparse
import pickle
...
class RawLayoutDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(label)):
            # 如果是 SVG (ID 0 或 3) 且面積較大 (例如大於畫面的 15%)
            # 這裡的面積計算是 width * height (bbox[i][2] * bbox[i][3])
            area = bbox[i][2] * bbox[i][3]
            if (label[i] == 0 or label[i] == 3) and area > 0.15:
