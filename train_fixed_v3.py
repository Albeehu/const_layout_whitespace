    ...
    for epoch in range(max_epoch):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            if iteration % args.vis_every == 0:
                # --- 過濾邏輯：隱藏指定 label --- 
                # 目標：生成/可視化時不要畫出 label 3、4 的框
                # 注意：to_dense_batch 回傳的 mask=True 表示「有效節點」
                # 但 Generator/Discriminator 與 save_image 通常使用 padding_mask=True 表示「要忽略/不畫」
                hide_ids = [3, 4]
                ...
                # fixed_mask: True=有效節點  -> padding_mask: True=padding/忽略
                vis_padding_mask = (~fixed_mask) | hide_mask

                # （可選）如果你希望連 Generator 也把 3/4 當作 padding，不產生那些框：
                fixed_label_vis = fixed_label.clone()
                fixed_label_vis[hide_mask] = 0  # 避免超出類別範圍 / 當作 background


                # 調試訊息：如果你在終端機看到這個，代表過濾邏輯有在跑
                if iteration == 0:
                    print(f"DEBUG: 正在生成圖片，已隱藏 label {hide_ids} 的框。")

                # --- 1. 生成 Real Sample ---
                if fixed_bbox_real is not None:
                    out_path_real = out_dir / 'real_samples.png'
                    save_image(fixed_bbox_real, fixed_label_vis, vis_padding_mask, 
                ...
                with torch.no_grad():
                    ...
                    bbox_vis = netG(fixed_z, fixed_label_vis, vis_padding_mask)
                    save_image(bbox_vis, fixed_label_vis, vis_padding_mask, 
