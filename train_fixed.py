...
def pairwise_overlap_loss(bbox_fake, label, padding_mask, ignore_ids=(FACE_ID, 3), eps=1e-8):
    ...
    for b in range(B):
        ...
        keep = torch.ones_like(labs, dtype=torch.bool)
        ...
        inter = box_intersection_area_xyxy(xyxy, xyxy)  # (K,K)
        # 去掉對角線
        inter = inter - torch.diag(torch.diagonal(inter))
        # normalize：用 min(area_i, area_j) 做比例，避免大框主宰
        area = box_area_xyxy(xyxy, eps=eps)        # (K,)
        denom = torch.minimum(area[:, None], area[None, :]).clamp(min=eps)
        overlap_ratio = (inter / denom).triu(1)    # 只取上三角
        losses.append(overlap_ratio.mean())
