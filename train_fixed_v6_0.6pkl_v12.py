...
"""
v12 完整修改版：
1. 實作預訓練權重載入 (Fine-tuning 邏輯)
2. 修正 ID 4 (Mask) 視覺化排除問題
3. 嚴格保留原本命名方式，避免 NameError
4. 整合延遲啟動與留白緩衝區
"""
import os
import argparse
...
def containment_loss(bbox_fake, label, padding_mask, inner_id=FACE_ID, outer_id=SVG_ID, lambda_cont=20.0):
    device = bbox_fake.device
    B, N, _ = bbox_fake.shape
...
def face_coverage_loss(bbox_fake, label, padding_mask, lambda_face=0.3):
    ...
    for b in range(B):
        ...
        is_container = (labels_b[other_m] == SVG_ID) | (labels_b[other_m] == BG_ID)
...
def pairwise_overlap_loss(bbox_fake, label, padding_mask):
    ...
    W[SVG_ID, FACE_ID], W[FACE_ID, SVG_ID] = 0.0, 0.0 
    W[1, 1], W[1, FACE_ID], W[FACE_ID, 1] = 25.0, 50.0, 50.0 # 文字互壓與文字壓臉重罰
...
def main():
    ...
    if os.path.exists(pretrained_path):
        ...
        # 1. 載入 Generator 權重 (過濾不匹配的層)
        ckpt_g = checkpoint['netG'] if 'netG' in checkpoint else checkpoint
        # 只保留名稱相同且 shape 相同的權重
        pretrained_g = {k: v for k, v in ckpt_g.items() if k in g_dict and v.size() == g_dict[k].size()}
        g_dict.update(pretrained_g)
        netG.load_state_dict(g_dict)

        # 2. 載入 Discriminator 權重
        ckpt_d = checkpoint['netD'] if 'netD' in checkpoint else {}
        pretrained_d = {k: v for k, v in ckpt_d.items() if k in d_dict and v.size() == d_dict[k].size()}
        d_dict.update(pretrained_d)
        netD.load_state_dict(d_dict)

        print(f"===> 載入成功！(忽略了 {len(ckpt_g)-len(pretrained_g)} 個不匹配的層)")
    ...
    for epoch in range(10000):
        for data in train_dataloader:
            ...
            loss_contain = containment_loss(bbox_fake, label, padding_mask, FACE_ID, SVG_ID)
            loss_align = alignment_loss(bbox_fake, padding_mask)
            
            # 尺寸限制：針對 ID 2(圖片容器) 與其他裝飾
            loss_img_size = element_size_loss(bbox_fake, label, padding_mask, [SVG_ID], 0.35)
