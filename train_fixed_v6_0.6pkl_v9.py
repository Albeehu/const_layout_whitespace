...
# 新增：針對不同 ID 設定不同的面積上限
def element_size_loss(bbox_fake, label, padding_mask, target_id, max_area, lambda_size=5.0):
    device = bbox_fake.device
    mask = (label == target_id) & (~padding_mask)
    if not mask.any():
...
def containment_loss(bbox_fake, label, padding_mask, inner_id=5, outer_id=2, lambda_cont=20.0):
    ...
    for b in range(B):
        valid = ~padding_mask[b]
        inner_m = (label[b] == inner_id) & valid
        outer_m = (label[b] == outer_id) & valid
        if not inner_m.any() or not outer_m.any(): continue
        
        # 取得人臉與圖片的 xyxy 座標
        in_xyxy = xywh_to_xyxy(bbox_fake[b][inner_m])  # [Ni, 4]
        out_xyxy = xywh_to_xyxy(bbox_fake[b][outer_m]) # [No, 4]

        # 對於每個臉，找到最靠近/重疊最多的圖片（這裡簡化為與所有圖片比對，取最小懲罰）
        # penalty: relu(外.x1 - 內.x1) + relu(內.x2 - 外.x2) ...
        p1 = torch.relu(out_xyxy[:, 0:1] - in_xyxy[:, 0].T) # [No, Ni]
        p2 = torch.relu(in_xyxy[:, 2].T - out_xyxy[:, 2:3]) 
        p3 = torch.relu(out_xyxy[:, 1:2] - in_xyxy[:, 1].T) 
        p4 = torch.relu(in_xyxy[:, 3].T - out_xyxy[:, 3:4])
        
        # 每個臉必須至少被「一個」圖片包含
        indiv_face_penalty = (p1 + p2 + p3 + p4).min(dim=0)[0]
        total_loss += indiv_face_penalty.mean()
        count += 1
...
def main():
    ...
    for epoch in range(int(1e+5)):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            # 新增：限制 Image (ID 0) 面積 < 20%
            loss_img_size = element_size_loss(bbox_fake, label, padding_mask, target_id=0, max_area=0.20, lambda_size=5.0)
            # 新增：限制 SVG (ID 2) 面積 < 10%
            loss_svg_size = element_size_loss(bbox_fake, label, padding_mask, target_id=2, max_area=0.10, lambda_size=10.0)
