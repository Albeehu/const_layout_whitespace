...
def style_scores_from_margins(L: float, R: float, T: float, B: float) -> Dict[str, float]:
    ...
    # Side whitespace: keep the original directional side-margin emphasis.
    S_side = h_max + 0.8 * (h_max - h_min) + 0.2 * v_mean

    # Top-bottom whitespace revised:
    # The previous version used v_max + 0.8 * (v_max - v_min),
    # which strongly rewarded extreme top/bottom imbalance.
    # This version rewards overall vertical whitespace first, and only gives
    # moderate additional credit to top-bottom difference. This avoids
    # unfairly penalizing balanced but visually acceptable vertical whitespace.
    S_tb = 0.6 * v_max + 0.4 * v_mean + 0.3 * (v_max - v_min) + 0.2 * h_mean
