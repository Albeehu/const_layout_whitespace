"""用 train_fixed_v6_0.6pkl_v38 改 v39 這版有問題 下版用v38改
    1. 讓模型不猜face位置
"""
import os
import argparse
...
class RawLayoutDataset(torch.utils.data.Dataset):
    ...
    2. 其他元件依優先權（Image > Text > SVG > BG > Mask）保留最多 max_nodes 個。
    3. Face 直接按原始順序取前 max_faces 個。
    4. Face 仍會合併回 node 序列，但訓練時可選擇「凍結人臉」：
       - Face 的 bbox 不讓模型學 / 不讓模型猜（永遠用 GT 覆蓋）
       - 同時可把 GT face bbox 餵進 face token 的 latent z，讓模型在生成其他元件時『知道』臉的位置
    """
...
def face_occlusion_ratio(
    bbox: torch.Tensor,
    label: torch.Tensor,
    padding_mask: torch.Tensor,
    face_id: int = FACE_ID,
    exclude_ids=(BG_ID, MASK_ID),
) -> torch.Tensor:
    """計算 face 被其他元件遮擋的比例（0~1）。

    - bbox: (B,N,4) cx,cy,w,h
    - label: (B,N)
    - padding_mask: (B,N) True = padding

    回傳：batch 平均遮擋比例（如果 batch 沒有 face，回傳 0）

    備註：這裡用『遮擋面積總和』近似 union（再 clamp 到 face 面積），
    目的主要是做訓練監控 / 是否有遮擋的判斷。
    """
    device = bbox.device
    B, N, _ = bbox.shape
    ...
    for b in range(B):
        ...
        other_m = valid & (label[b] != face_id)
        for eid in exclude_ids:
            other_m &= (label[b] != eid)
        if not other_m.any():
            ratios.append(torch.zeros((), device=device))
            continue

        f_xyxy = xywh_to_xyxy(bbox[b][f_m])          # [F,4]
        o_xyxy = xywh_to_xyxy(bbox[b][other_m])      # [K,4]
        inter = box_intersection_area_xyxy(o_xyxy, f_xyxy)  # [K,F]
        f_area = box_area_xyxy(f_xyxy, eps=1e-6)            # [F]

        # 近似 union：sum 後 clamp 到 face 面積，避免 > 1
        occ_area = inter.sum(dim=0).clamp(max=f_area)
        ratios.append((occ_area / f_area).clamp(0, 1).mean())

    return torch.stack(ratios).mean() if ratios else torch.tensor(0.0, device=device)
...
def main():
    ...
    # === Face 行為控制 ===
    parser.add_argument('--freeze_face', type=int, default=1)          # 1=Face bbox 永遠用 GT 覆蓋，不讓模型猜
    parser.add_argument('--face_cond_z', type=int, default=1)          # 1=把 GT face bbox 塞進 face token 的 z，讓模型「知道」臉位置
    ...
    # 若凍結人臉，anchor loss 就沒有意義（Face 不再由模型輸出來對齊 GT）
    if args.freeze_face:
    ...
    # --- Robust sanitize for fixed sample ---
    # layoutganpp.emb_label 需要 Long/Int indices
    fixed_label = fixed_label.long()
    # 我們這裡的 fixed_mask 代表「有效元素 mask」，後面會用 ~fixed_mask 當 padding_mask
    fixed_mask = fixed_mask.bool()

    # 有些 fixed_sample 可能是用不同 batch_size / max_elem 產生，做一次安全對齊
    # fixed_z: (B, N, z_dim), fixed_label/fixed_mask: (B, N)
    B = min(fixed_z.size(0), fixed_label.size(0), fixed_mask.size(0))
    fixed_z = fixed_z[:B]
    fixed_label = fixed_label[:B]
    fixed_mask = fixed_mask[:B]

    N = min(fixed_z.size(1), fixed_label.size(1), fixed_mask.size(1))
    fixed_z = fixed_z[:, :N]
    fixed_label = fixed_label[:, :N]
    fixed_mask = fixed_mask[:, :N]
    
    # 視覺化時：Face 不畫出來（但訓練/計算遮擋仍會用到 face bbox）
    ...
    for epoch in range(10000):
        for data in train_dataloader:
            ...
            # === 讓模型『看到』真實 face 位置（但不需要模型猜） ===
            face_tok_m = (label == FACE_ID) & (~padding_mask)
            if args.face_cond_z and face_tok_m.any():
                if args.latent_size < 4:
                    raise ValueError("face_cond_z 需要 latent_size >= 4，因為要塞 cx,cy,w,h")
                z_face = torch.zeros_like(z)
                z_face[..., :4] = bbox_real.detach()  # 不讓 GT bbox 參與梯度
                z = torch.where(face_tok_m.unsqueeze(-1), z_face, z)

            # Update G
            netG.train(); netG.zero_grad()
            bbox_fake = torch.clamp(netG(z, label, padding_mask), 0, 1)

            # === Face 永遠不讓模型猜：直接用 GT 覆蓋 ===
            if args.freeze_face and face_tok_m.any():
                bbox_fake = torch.where(face_tok_m.unsqueeze(-1), bbox_real, bbox_fake)
            ...
            # Face anchor 只在「沒有凍結人臉」的情況下才有意義
            if (not args.freeze_face) and (curr_lambda_anchor > 0):
                loss_G += face_anchor_loss(bbox_fake, label, padding_mask, face_pos, face_mask, lambda_anchor=curr_lambda_anchor)
            ...
            if iteration % 100 == 0:
                occ = face_occlusion_ratio(bbox_fake.detach(), label, padding_mask)
                print(f'[{iteration}] Loss_D: {loss_D.item():.4f} Loss_G: {loss_G.item():.4f} WS:{curr_lambda_ws:.3f} OV:{curr_lambda_ov:.3f} ALIGN:{curr_lambda_align:.3f} FACE:{curr_lambda_face:.3f} CONT:{curr_lambda_cont:.1f} ANCH:{curr_lambda_anchor:.1f} OCC:{occ.item():.3f}')
                writer.add_scalar('train/face_occlusion_ratio', occ.item(), iteration)
            ...
            if iteration % args.vis_every == 0:
                ...
                with torch.no_grad():
                    ...
                    # 建立視覺化遮罩：排除 BG/MASK/FACE（face 不畫出來）
                    ...
                    for b in range(fixed_label.size(0)):
                        for n in range(fixed_label.size(1)):
                            ...
                            if fid == BG_ID or fid == MASK_ID or fid == FACE_ID:
                    ...
                    # 自動補色：確保 colors 覆蓋到 max label（IMG_ID 才不會被顏色表不足而濾掉）
                    max_id = int(fixed_label.max().item())
                    if max_id >= len(colors_pil):
                        import random
                        for i in range(len(colors_pil), max_id + 1):
                            rng = random.Random(i)
                            colors_pil.append((rng.randint(30,225), rng.randint(30,225), rng.randint(30,225)))

                    print("IMG_ID", IMG_ID,
                        "n_img", int(((fixed_label == IMG_ID) & fixed_mask).sum().item()),
                        "n_img_vis", int(((fixed_label == IMG_ID) & final_vis_mask).sum().item()),
                        "max_label", int(fixed_label.max().item()),
                        "n_colors", len(colors_pil))

                    img_m = (fixed_label == IMG_ID) & fixed_mask
                    img_area = (bbox_vis[..., 2] * bbox_vis[..., 3])[img_m]
                    print("n_img:", int(img_m.sum().item()),
                        "img_area mean/min/max:",
                        float(img_area.mean().item()), float(img_area.min().item()), float(img_area.max().item()))

                    save_image(bbox_vis, fixed_label, final_vis_mask, colors_pil, vis_save_path)
                    print(f"===> 已儲存預覽圖 (face 不畫出來): {vis_save_path}")
                netG.train()
            iteration += 1
