"""用 train_fixed_v6_0.6pkl_v46 改 v47
    1. grid_alignment_loss_xy 先不看svg
    2. loss_text_width 防止變成細線
    3. 最小面積懲罰放鬆
    4. 強化文字相關重疊懲罰
"""
import os
...
def pairwise_overlap_loss(bbox_fake, label, padding_layout, big_img_thr: float = 0.90):
    ...
    W[TEXT_ID, IMG_ID],  W[IMG_ID, TEXT_ID]  = 8.0, 8.0
    W[TEXT_ID, SVG_ID],  W[SVG_ID, TEXT_ID]  = 12.0, 12.0
    W[SVG_ID, SVG_ID] = 4.0
...
def main():
    ...
    for epoch in range(10000):
        for data in train_dataloader:
            ...
            if text_mask.any():
                # 文字寬度保護，防止變成細線
                loss_text_width = torch.relu(0.18 - bbox_fake[text_mask][:, 2]).mean()
                loss_G += (loss_text_width * 50.0)
