...
def main():
    ...
    parser.add_argument('--lambda_ws', type=float, default=0.02)
    parser.add_argument('--wr_min', type=float, default=0.6)
    parser.add_argument('--lambda_ov', type=float, default=0.5)
    ...
    for epoch in range(int(1e+5)):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            if isinstance(data, dict):
                ...
                loss_D += 1.0 * F.mse_loss(bbox_recon, valid_bbox_real)
            else:
                # 原始 Crello 格式
                loss_D += F.cross_entropy(logit_cls.view(-1, num_label), target_y.view(-1), ignore_index=-1)
                loss_D += 1.0 * F.mse_loss(bbox_recon, data.x)
            ...
            if iteration % args.fid_every == 0:
                ...
                l_f[m] = 0
                # 確保 label 索引在 Embedding 範圍內
                l_f = torch.clamp(l_f, 0, fid_train.model.emb_label.num_embeddings - 1)
                fid_train.collect_features(bbox_fake.detach(), l_f, p_f)
                fid_train.collect_features(bbox_real, l_f, p_f, real=True)
            ...
            if iteration % args.vis_every == 0:
                netG.eval(); vis_mask = fixed_mask & (~((fixed_label == 3) | (fixed_label == 4)))
                with torch.no_grad():
                    bbox_vis = netG(fixed_z, fixed_label, ~fixed_mask)
                    save_image(bbox_vis, fixed_label, vis_mask, train_dataset.colors, out_dir / f'fake_{iteration:07d}.png')
                netG.train()
            iteration += 1
