...
"""
whitespace_metric_v10_stronger_coarse.py
...
Recommended stronger coarse setting for 128x128 layouts:
    --coarse_preset strong

Equivalent parameters:
    --dilation_px 3 --block_size 8 --block_occ_ratio 0.10
...
def preprocess_whitespace_mask(
    occ_mask: np.ndarray,
    use_coarse_ws: bool = True,
    dilation_px: int = 3,
    block_size: int = 8,
    block_occ_ratio: float = 0.10,
) -> np.ndarray:
    ...
    Args:
        ...
        dilation_px:
            ...
            For 128x128 layouts, use 2~4; stronger default is 3.
        block_size:
            Coarse block size. For 128x128 layouts, use 4, 8, or 16; stronger default is 8.
        block_occ_ratio:
    ...
    Returns:
        whitespace_mask: np.uint8 array, 1 = whitespace, 0 = content.
            ...
            Stronger coarse settings reduce resolution more, e.g. 128x128 -> 16x16 when block_size=8.
    """
...
def summarize_metrics(metrics_list: List[Dict[str, float]]) -> Dict[str, float]:
    ...
    if metrics_list:
        ...
        for k in [
            "use_coarse_ws", "coarse_preset_id", "dilation_px", "block_size", "block_occ_ratio",
...
def format_summary_block(name: str, summary: Dict[str, float]) -> str:
    ...
    config_keys = [
        "target_style", "use_coarse_ws", "coarse_preset_id", "dilation_px", "block_size",
...
def save_metrics_csv(metrics_list: List[Dict[str, float]], output_csv: Path) -> None:
    ...
    fieldnames = [
        ...
        "use_coarse_ws", "coarse_preset_id", "dilation_px", "block_size", "block_occ_ratio", "connectivity",
        "metric_h", "metric_w",
    ]
...
def apply_coarse_preset(args: argparse.Namespace) -> argparse.Namespace:
    """Apply named coarse-whitespace presets.

    Presets for 128x128 masks:
    - gentle:      128 -> 32 grid, mild gap removal
    - normal:      128 -> 32 grid, original v9-like setting
    - strong:      128 -> 16 grid, stronger precision reduction
    - very_strong: 128 -> 8 grid, very coarse, only very large whitespace survives
    - custom:      use the explicit --dilation_px / --block_size / --block_occ_ratio values
    """
    preset = str(args.coarse_preset).lower().strip()
    presets = {
        "gentle": dict(dilation_px=1, block_size=4, block_occ_ratio=0.25),
        "normal": dict(dilation_px=2, block_size=4, block_occ_ratio=0.15),
        "strong": dict(dilation_px=3, block_size=8, block_occ_ratio=0.10),
        "very_strong": dict(dilation_px=4, block_size=16, block_occ_ratio=0.08),
    }
    if preset == "custom":
        return args
    if preset not in presets:
        raise ValueError(f"Unsupported coarse_preset={args.coarse_preset}")
...
def parse_args() -> argparse.Namespace:
    ...
    # This v10 file enables coarse whitespace by default because the goal is to
    # reduce precision more strongly than v9. Use --no_coarse_ws to recover
    # pixel-level v8/v9 behavior.
    parser.set_defaults(use_coarse_ws=True)
    parser.add_argument(
        "--use_coarse_ws",
        dest="use_coarse_ws",
        action="store_true",
        help="Enable content dilation + block downsampling before computing WR/LWR. Enabled by default in v10.",
    )
    parser.add_argument(
        "--no_coarse_ws",
        dest="use_coarse_ws",
        action="store_false",
        help="Disable coarse preprocessing and compute WR/LWR on the original 128x128 mask.",
    )
    parser.add_argument(
        "--coarse_preset",
        type=str,
        default="strong",
        choices=["gentle", "normal", "strong", "very_strong", "custom"],
        help=(
            "Named precision-reduction preset. "
            "gentle=(dilation=1, block=4, occ=0.25), "
            "normal=(2,4,0.15), strong=(3,8,0.10), "
            "very_strong=(4,16,0.08). Use custom to keep manual values."
        ),
    )
    parser.add_argument(
        "--dilation_px",
        type=int,
        default=3,
        help="Content dilation size. Stronger default is 3 for 128x128 layouts.",
    )
    parser.add_argument(
        "--block_size",
        type=int,
        default=8,
        help="Block size for coarse whitespace map. Stronger default is 8, so 128x128 becomes 16x16.",
    )
    parser.add_argument(
        "--block_occ_ratio",
        type=float,
...
def main() -> None:
    ...
    print(f"[INFO] coarse_preset: {args.coarse_preset}")
