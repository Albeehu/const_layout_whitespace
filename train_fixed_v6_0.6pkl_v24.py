"""用 train_fixed_v6_0.6pkl_v23 改 v24
    1. loss_min_size忘記加入loss
...
class RawLayoutDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
            ...
            final_f_label = []
            final_f_bbox = []
            img_boxes = o_bbox[o_label == IMG_ID]
            ...
            # --- [修正點 A: 初始化回傳給 Face Anchor Loss 的數據] ---
    ...
    for epoch in range(10000):
        for data in train_dataloader:
            ...
            if img_mask.any():
                ...
                loss_G += (loss_img_min_area * 150.0) # 給予極高權重，確保圖片長大

            # B. 文字寬度保護 (防止文字縮成一條垂直線)
            text_mask = (label == TEXT_ID) & valid_mask
            if text_mask.any():
                # 強迫文字框寬度至少要有 0.25 (畫布的 1/4)
                text_widths = bbox_fake[text_mask][:, 2]
                loss_text_width = torch.relu(0.25 - text_widths).mean()
                loss_G += (loss_text_width * 80.0)
