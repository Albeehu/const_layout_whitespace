...
def main():
    ...
    for epoch in range(10000):
        for data in train_dataloader:
            ...
            # --- [修改後的排版邏輯：實現單側排版與大面積留白] ---
            
            # 1. 取得所有有效元件的座標
            valid_mask = ~padding_mask
            # 取得右邊界座標 x2 (cx + w/2)
            x2_all = (bbox_fake[:, :, 0] + bbox_fake[:, :, 2] / 2).clamp(0, 1)
            # 取得中心 X 座標
            x_centers = bbox_fake[:, :, 0]

            # 2. [關鍵：強制右對齊] 
            # 不要給模型 0.1 或 0.5 的選擇，直接懲罰不靠近 0.9 (右側) 的行為
            if mask.any():
                # 強迫所有元件的右邊界靠近 0.9，這會自動在左側留出大片空白
                loss_grid = torch.abs(x2_all[mask] - 0.9).mean()
                loss_G += (loss_grid * 100.0) # 提高權重，這是留白的關鍵

                # 3. [關鍵：水平群聚] 
                # 計算所有元件中心 X 的標準差，強迫它們站在同一條垂直線上，消除散亂感
                loss_group_x = x_centers[mask].std()
                loss_G += (loss_group_x * 80.0) 

            # 4. [升級：圖片階層感]
            img_mask = (label == IMG_ID) & valid_mask
            if img_mask.any():
                # 參考圖中的大型元素通常位於中上部，給予圖片 y 軸位置偏好
                img_centers_y = bbox_fake[img_mask][:, 1]
                loss_img_pos = (img_centers_y - 0.4).pow(2).mean() 
                loss_G += (loss_img_pos * 50.0)
            ...
            if text_mask.any():
                ...
                # 參考圖的文字通常有一定寬度，限制最小寬度 0.25
                loss_text_width = torch.relu(0.25 - text_w).mean()
