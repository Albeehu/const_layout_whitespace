"""用 train_fixed_v6_0.6pkl_v33 改 v34
    1. 刪除不在image內的face篩選邏輯
    2. 修改 loss_style
"""
import os
import argparse
...
def main():
    ...
    for epoch in range(10000):
        for data in train_dataloader:
            ...
            B = label.size(0)
            style_tags = torch.arange(B, device=device) % 4 
            ...
            if style_idx == 0: # Style A: 右靠 (左側留白)
                loss_style = torch.abs(x2_f[mask] - 0.85).mean()
            elif style_idx == 1: # Style B: 置中
                loss_style = torch.abs(cx_f[mask] - 0.5).mean()
            elif style_idx == 2: # Style C: 上下
                img_m = (label == IMG_ID) & mask
                txt_m = (label == TEXT_ID) & mask
                ...
                loss_style = torch.abs(x2_f[mask] - 0.95).mean() + torch.abs(y2_f[mask] - 0.95).mean()

            loss_G += (loss_style * 150.0)

            # [修正點]：使用剛剛定義好的 x1_f 和 cx_f 計算一致性
            if mask.any():
                # 強迫元件在 X 軸對齊，解決排版散亂問題
                loss_consistency = x1_f[mask].std() + cx_f[mask].std()
                loss_G += (loss_consistency * 80.0)
