"""用 train_fixed_v6_0.6pkl_v18 改 v19
    1. 修改 face_coverage_loss 與 containment_loss 讓人臉在image內
    2. 新增face_anchor_loss 強迫生成器預測的人臉框
...
def containment_loss(bbox_fake, label, padding_mask, outer_id=IMG_ID, inner_id=FACE_ID, lambda_cont=30.0):
    """
    正確邏輯：生成的人臉(inner_id)必須被包含在生成的圖片(outer_id)中。
    """
    device = bbox_fake.device
    B, N, _ = bbox_fake.shape
    ...
    for b in range(B):
        ...
        out_xyxy = xywh_to_xyxy(bbox_fake[b][out_m]) # 生成的圖片 [M, 4]
        in_xyxy  = xywh_to_xyxy(bbox_fake[b][in_m])  # 生成的人臉 [F, 4]

        # 每個 face 尋找最靠近它的圖片容器
        # p1~p4 計算 face 超出 image 邊界的距離
        p1 = torch.relu(out_xyxy[:, 0:1] - in_xyxy[:, 0].T) # face左邊界超出image左邊
        p2 = torch.relu(in_xyxy[:, 2].T - out_xyxy[:, 2:3]) # face右邊界超出image右邊
        p3 = torch.relu(out_xyxy[:, 1:2] - in_xyxy[:, 1].T) # face上邊界超出image上邊
        p4 = torch.relu(in_xyxy[:, 3].T - out_xyxy[:, 3:4]) # face下邊界超出image下邊
        
        penalty = (p1 + p2 + p3 + p4) # [M, F]
        
        # 每個 face 至少要被一個 image 包含，取 min penalty
