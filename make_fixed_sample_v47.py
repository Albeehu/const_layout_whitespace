...
class RawLayoutDataset:
    """
    Fixed-sample generation dataset that matches your training rules:
    - Face tokens do NOT count toward max_nodes
    - Keep non-face tokens by priority only (IMG > TEXT > SVG > BG > MASK),
      tie within same priority randomized (but deterministic per idx using base_seed+idx)
    - Big SVG (area>0.15) relabel as IMG
    - IMPORTANT FIX:
    ...
    def __getitem__(self, idx: int) -> SamplePack:
        ...
        # choose max_nodes by priority only; tie random (deterministic via rng)
        if n > self.max_nodes:
...
def main():
    ...
    ap.add_argument("--out", type=str, default="fixed_sample_v47.pt")
