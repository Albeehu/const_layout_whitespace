...
def postprocess_boxes(
    ...
    if svg_roles and style is not None and rng is not None:
...
def write_metrics_csv(out_dir: Path, metas: list[dict], labels_list: list[np.ndarray]):
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writeheader()
        for i, (meta, labels) in enumerate(zip(metas, labels_list)):
            row = {
                "index": i,
...
def write_summary_txt(out_dir: Path, args, metas: list[dict], labels_list: list[np.ndarray]):
    txt_path = out_dir / "summary.txt"
    scores = np.array([float(m.get("score", 0.0)) for m in metas], dtype=np.float64)
    overlaps = np.array([float(m.get("overlap", 0.0)) for m in metas], dtype=np.float64)
    areas = np.array([float(m.get("area", 0.0)) for m in metas], dtype=np.float64)

    def _line(name, arr):
        return f"{name}: mean={arr.mean():.6f}, min={arr.min():.6f}, max={arr.max():.6f}"
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"n: {args.n}\n")
        f.write(f"k: {args.k}\n")
        f.write(f"resume_ckpt: {args.resume_ckpt}\n")
        f.write(f"max_fg_area: {args.max_fg_area}\n")
        f.write(f"max_elems: {args.max_elems}\n")
        f.write(f"add_face: {args.add_face}\n")
        f.write(f"svg_small_prob: {args.svg_small_prob}\n")
        f.write(f"frame_band_margin: {args.frame_band_margin}\n")
        f.write(f"frame_target_w: {args.frame_target_w}\n")
        f.write(f"frame_target_h: {args.frame_target_h}\n")
        f.write(f"frame_max_ar: {args.frame_max_ar}\n")
        if args.labels is not None:
            f.write(f"fixed labels: {' '.join(map(str, args.labels))}\n")
        f.write("\n")
        f.write(_line("score", scores) + "\n")
        f.write(_line("overlap", overlaps) + "\n")
        f.write(_line("area", areas) + "\n")

        if args.style == "frame":
            for key in [
                "frame_penalty", "frame_band_penalty", "frame_min_margin_penalty",
                "frame_balance_penalty", "frame_center_penalty",
                "frame_width_penalty", "frame_height_penalty",
                "frame_aspect_penalty", "frame_left", "frame_right",
                "frame_top", "frame_bottom", "frame_union_w",
                "frame_union_h", "frame_aspect",
            ]:
                arr = np.array([float(m.get(key, 0.0)) for m in metas], dtype=np.float64)
                f.write(_line(key, arr) + "\n")

        f.write("\nTop samples by score\n")
        f.write("--------------------\n")
        for rank, idx in enumerate(topk, start=1):
            meta = metas[int(idx)]
            labels = labels_list[int(idx)]
            f.write(
                f"#{rank}: {idx:04d}.png | labels={' '.join(map(str, labels.tolist()))} | "
                f"score={float(meta.get('score', 0.0)):.6f} | "
                f"overlap={float(meta.get('overlap', 0.0)):.6f} | "
                f"area={float(meta.get('area', 0.0)):.6f}"
            )
            if args.style == "frame":
                f.write(
                    f" | L={float(meta.get('frame_left', 0.0)):.4f}"
                    f" R={float(meta.get('frame_right', 0.0)):.4f}"
                    f" T={float(meta.get('frame_top', 0.0)):.4f}"
                    f" B={float(meta.get('frame_bottom', 0.0)):.4f}"
                    f" | uw={float(meta.get('frame_union_w', 0.0)):.4f}"
                    f" uh={float(meta.get('frame_union_h', 0.0)):.4f}"
                    f" ar={float(meta.get('frame_aspect', 0.0)):.4f}"
                )
            f.write("\n")
...
def main():
    ...
    write_metrics_csv(out_dir, metas, labels_list)
    write_summary_txt(out_dir, args, metas, labels_list)
