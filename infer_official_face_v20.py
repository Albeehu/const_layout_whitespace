...
def render_layout_preview_exact(
    ...
    for box, lb in zip(boxes, labels):
        ...
        draw.rectangle(
            [px1, py1, px2, py2],
            fill=(*base_color, fill_alpha),
            outline=(*base_color, 255),
            width=border_width,
        )

        tag = str(lb)
        tx = px1 + max(2, border_width)
        ty = py1 + max(2, border_width)
        draw.text((tx, ty), tag, fill=(*base_color, 255))
...
def main():
    ...
    parser.add_argument("--preview_border_width", type=int, default=8,
                        help="layout preview 的框線粗細，數字越大框線越明顯")
    parser.add_argument("--preview_fill_alpha", type=int, default=0,
                        help="layout preview 的色塊透明度，0=只畫框線，255=實心")
