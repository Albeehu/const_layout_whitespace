"""用 train_fixed_v6_0.6pkl_v35 改 v36
    1. 修改 loss以及overlap
"""
import os
import argparse
...
def main():
    ...
    for epoch in range(1000):
        for data in train_dataloader:
            ...
            # --- [正確的 Loss 加總邏輯] ---
            loss_G = F.softplus(-netD(bbox_fake, label, padding_mask)).mean() * 1.5
            valid = ~padding_mask
            ...
            if style_idx == 0: # 左右排版
                loss_style = torch.relu(0.4 - x1_f[mask].min()) # 只要左邊距 > 0.4 就不再懲罰
            elif style_idx == 3: # 角落排版
                loss_style = torch.relu(0.8 - y2_f[mask].mean()) # 只要整體靠下部即可
            loss_G += (loss_style * 80.0) # 調降權重避免壓過 Overlap

            # 加強重疊與美學損失
            loss_ov = pairwise_overlap_loss(bbox_fake, label, padding_mask)
            # 將 Overlap 權重設為動態，隨 iteration 增加強度
            curr_ov_weight = args.lambda_ov * min(2.0, iteration / 10000.0)
            loss_G += (loss_ov * curr_ov_weight * 10.0) # 強力分開元件
