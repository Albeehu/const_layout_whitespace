"""用 train_fixed_v6_0.6pkl_v56 改 v57
1. 改loss做四周留白
"""
import os
import csv
...
def poster_template_loss(
    ...
    for b in range(bbox_fake.size(0)):
        ...
        if template == "center_vertical":
            ...
            # Poster reference: slim centered content band with large frame whitespace.
            left_margin = x1_all.min()
            right_margin = 1.0 - x2_all.max()
            ...
            loss_b += ((union_cy - 0.53) ** 2) * 1.5
            loss_b += torch.relu(union_w - 0.34) * 4.0
            loss_b += torch.relu(union_h - 0.72) * 1.5
            loss_b += ((left_margin - right_margin) ** 2) * 3.0
            loss_b += torch.relu(0.22 - left_margin) * 4.0
            loss_b += torch.relu(0.22 - right_margin) * 4.0
            loss_b += torch.relu(0.10 - top_margin) * 2.0
            loss_b += torch.relu(0.12 - bottom_margin) * 2.5

            if text_m.any():
                t = boxes[text_m]
                loss_b += ((t[:, 0] - 0.50) ** 2).mean() * 3.5
                loss_b += torch.relu(t[:, 2] - 0.18).mean() * 3.0
                loss_b += torch.relu(t[:, 1].mean() - 0.54) * 1.5

            if graphic_m.any():
                g = boxes[graphic_m]
                g_area = g[:, 2] * g[:, 3]
                k = torch.argmax(g_area)
                main_g = g[k]
                loss_b += ((main_g[0] - 0.50) ** 2) * 2.5
                loss_b += ((main_g[1] - 0.72) ** 2) * 1.5
                loss_b += torch.relu(main_g[2] - 0.36) * 2.5
                loss_b += torch.relu(main_g[3] - 0.28) * 2.5

                if text_m.any():
                    text_bottom = (boxes[text_m][:, 1] + 0.5 * boxes[text_m][:, 3]).max()
                    graphic_top = main_g[1] - 0.5 * main_g[3]
                    loss_b += torch.relu(text_bottom + 0.05 - graphic_top) * 2.5
...
def frame_vertical_role_loss(
    ...
    """Role-aware loss for a narrow, centered column with large margins on all sides.

    Target style reference: poster-like layout with a slim vertical content band,
    balanced left/right whitespace, generous top/bottom whitespace, text stacked in
    the upper-middle area, and the main graphic placed lower in the canvas.
    """
    device = bbox_fake.device
    losses = []
    ...
    for b in range(bbox_fake.size(0)):
        ...
        boxes = bbox_fake[b]
        valid_boxes = boxes[valid]
        valid_xyxy = xywh_to_xyxy(valid_boxes)
        L, R, T, Bm, cw, ch, ccx, ccy = _margin_stats_from_valid_boxes(valid_xyxy)
        ...
        # 1) Whole content block should be a narrow centered column.
        loss_b += ((ccx - 0.50) ** 2) * 4.0
        loss_b += ((ccy - 0.53) ** 2) * 1.5
        loss_b += torch.relu(cw - 0.34) * 4.5
        loss_b += torch.relu(ch - 0.72) * 2.0

        # 2) Large and balanced frame whitespace on all sides.
        loss_b += (L - R).pow(2) * 3.0
        loss_b += torch.relu(0.22 - L) * 4.0
        loss_b += torch.relu(0.22 - R) * 4.0
        loss_b += torch.relu(0.10 - T) * 2.5
        loss_b += torch.relu(0.12 - Bm) * 3.0

        # 3) Text should form a slim vertical stack near the upper-middle area.
        if text_m.any():
            t = boxes[text_m]
            tx = t[:, 0]
            ty = t[:, 1]
            ...
            loss_b += ((tx - 0.50) ** 2).mean() * 3.5
            loss_b += torch.relu(tw - 0.18).mean() * 3.0
            loss_b += torch.relu(0.018 - th).mean() * 2.0

            # Encourage vertical stacking with limited horizontal spread.
            t_xyxy = xywh_to_xyxy(t)
            tL, tR, tT, tB, tcw, tch, tccx, tccy = _margin_stats_from_valid_boxes(t_xyxy)
            loss_b += ((tccx - 0.50) ** 2) * 3.0
            loss_b += torch.relu(tcw - 0.14) * 3.0
            loss_b += torch.relu(tccy - 0.52) * 1.5
            loss_b += torch.relu(0.18 - tT) * 1.5

        # 4) Main graphic should sit in the lower-middle area and stay compact.
        if graphic_m.any():
            g = boxes[graphic_m]
            g_area = g[:, 2] * g[:, 3]
            k = torch.argmax(g_area)
            main_g = g[k]
            loss_b += ((main_g[0] - 0.50) ** 2) * 2.5
            loss_b += ((main_g[1] - 0.72) ** 2) * 1.8
            loss_b += torch.relu(main_g[2] - 0.36) * 2.5
            loss_b += torch.relu(main_g[3] - 0.28) * 2.5

            if text_m.any():
                # Keep the main graphic below the text block.
                t = boxes[text_m]
                text_bottom = (t[:, 1] + 0.5 * t[:, 3]).max()
                graphic_top = main_g[1] - 0.5 * main_g[3]
                loss_b += torch.relu(text_bottom + 0.05 - graphic_top) * 3.0
...
def _get_dynamic_region_cfg(template: str):
    ...
    cfg = {
        ...
        "frame_vertical": {
            "cx": 0.50, "cy": 0.53,
            "base_w": 0.10, "base_h": 0.18,
            "count_w": 0.035, "count_h": 0.060,
            "area_w": 0.35, "area_h": 0.55,
            "min_w": 0.14, "max_w": 0.40,
            "min_h": 0.22, "max_h": 0.74,
            "dir_w": 0.0, "dir_h": 0.0,
            "dir_margin": 0.00,
        },
        "right_column": {
        ...
        for data in train_dataloader:
            ...
            if curr_lambda_align > 0:
                if args.layout_template == 'center_vertical':
                    ...
                    gx = [0.50]
                    gy = [0.12, 0.22, 0.34, 0.48, 0.64, 0.80]
                    ew, cw = 0.35, 1.35
