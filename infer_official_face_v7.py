...
def score_layout(
    ...
    # 1. 獲取當前邊界 (L:左, R:右, T:上, B:下)
    x1, y1, x2, y2 = union_bbox(boxes)
    L, R, T, B = x1, 1.0 - x2, y1, 1.0 - y2
    
    # 2. 針對不同 style 設定目標分數
    # 這裡的邏輯是：如果該 style 期望某邊留白大，我們就獎勵那個邊界值較高
    if style == "side":
        # side 期望左或右留白大：獎勵 L 或 R 其中之一過大
        pos_pen = -1.5 * max(L, R) - 1.0 * abs(T - B)
    elif style == "tb":
        # tb 期望上或下留白大：獎勵 T 或 B 其中之一過大
        pos_pen = -1.5 * max(T, B) - 1.0 * abs(L - R)
    elif style == "frame":
        # frame 期望四邊均衡但允許微偏移
        pos_pen = 2.0 * abs(L - R) + 2.0 * abs(T - B)
    elif style == "hybrid":
        # hybrid 期望角落留白：獎勵邊界同時偏小（主體被推向角落）
        pos_pen = -2.0 * (L + R + T + B)
    else:
        pos_pen = 0.0

    # 3. 額外獎勵：保持內容不貼邊 (0.05 是容忍閾值)
    edge_safety = max(0.0, 0.05 - L) + max(0.0, 0.05 - R) + max(0.0, 0.05 - T) + max(0.0, 0.05 - B)
    
    # 4. 加入與 whitespace_metric 對齊的權重
    score = (
        w_ov * ov + 
        w_area * area_violation + 
        w_pos * pos_pen + 
        20.0 * edge_safety + # 強化邊緣防護
        w_face_conf * face_conflict_penalty(boxes, labels) +
        w_svg * svg_role_penalty(boxes, labels, svg_roles)
    )
    
    # meta 資料記錄 (用於 csv 分析)
    meta = {"score": float(score), "pos_penalty": float(pos_pen), "dominant_style": style}
    return score, meta
