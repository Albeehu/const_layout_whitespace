"""用 train_fixed_v6_0.6pkl_v31 改 v32
    1. 刪除alignment_loss(bbox_fake, padding_mask)
    2. 改main的三種風格
"""
import os
import argparse
...
def main():
    ...
    for epoch in range(10000):
        for data in train_dataloader:
            ...
            style_idx = (iteration // 1000) % 4 
            
            x1 = (bbox_fake[:, :, 0] - bbox_fake[:, :, 2] / 2).clamp(0, 1)
            x2 = (bbox_fake[:, :, 0] + bbox_fake[:, :, 2] / 2).clamp(0, 1)
            y1 = (bbox_fake[:, :, 1] - bbox_fake[:, :, 3] / 2).clamp(0, 1)
            y2 = (bbox_fake[:, :, 1] + bbox_fake[:, :, 3] / 2).clamp(0, 1)

            loss_style = torch.tensor(0.0, device=device)
            if style_idx == 0: # S_side (極端右靠，左側留白)
                loss_style = torch.abs(x2[mask] - 0.90).mean()
            elif style_idx == 1: # S_frame (中心平衡，四周留白)
                loss_style = torch.abs(bbox_fake[mask][:, 0] - 0.5).mean()
            elif style_idx == 2: # S_tb (上下留白)
                img_m = (label == IMG_ID) & mask
                if img_m.any():
                    # 強迫圖片縮減高度並置中 y 軸，留出上下空間
                    loss_style = torch.abs(bbox_fake[img_m][:, 1] - 0.5).mean()
            elif style_idx == 3: # Corner (右下角落)
                loss_style = torch.abs(x2[mask] - 0.95).mean() + torch.abs(y2[mask] - 0.95).mean()

            loss_G += (loss_style * 150.0) # 提高權重，強迫破壞置中慣性
