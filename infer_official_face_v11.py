...
def whitespace_proxy_score(boxes: np.ndarray, labels: np.ndarray) -> float:
    """Fast proxy for whitespace_metric_v7 DWS_max/S_max during best-of-k rerank.

    It ignores face label 5, then rewards layouts with one dominant side/top/bottom/
    corner whitespace region while also discouraging overly scattered objects.
    """
    fg = boxes[labels != 5]
    if len(fg) == 0:
        return 0.0
    ...
    edge_thr = 0.055
    h_edge_penalty = 1.0 if h_min >= edge_thr else 0.65 + 0.35 * h_min / max(edge_thr, 1e-8)
    v_edge_penalty = 1.0 if v_min >= edge_thr else 0.65 + 0.35 * v_min / max(edge_thr, 1e-8)
    ...
    union_w = max(0.0, x2 - x1)
    union_h = max(0.0, y2 - y1)
    compact_bonus = 1.0 - 0.35 * max(0.0, union_w - 0.55) - 0.35 * max(0.0, union_h - 0.65)

    S_max = max(S_side, S_tb, S_corner)
    return float(np.clip(S_max * compact_bonus, 0.0, 1.0))
