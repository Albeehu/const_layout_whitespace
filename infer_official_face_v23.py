...
def _box_area_xywh(box) -> float:
    return float(max(float(box[2]), 0.0) * max(float(box[3]), 0.0))


def _is_svg_text_pair(label_a: int, label_b: int) -> bool:
    return {int(label_a), int(label_b)} == {0, 1}


def _is_svg_image_pair(label_a: int, label_b: int) -> bool:
    return {int(label_a), int(label_b)} == {0, 2}


def overlap_allowance_ratio(
    label_a: int,
    label_b: int,
    svg_text_allowance_ratio: float = 0.18,
    svg_image_allowance_ratio: float = 0.12,
) -> float:
    if _is_svg_text_pair(label_a, label_b):
        return float(max(svg_text_allowance_ratio, 0.0))
    if _is_svg_image_pair(label_a, label_b):
        return float(max(svg_image_allowance_ratio, 0.0))
    return 0.0


def overlap_penalty_weight(
    label_a: int,
    label_b: int,
    svg_text_overlap_weight: float = 0.35,
    svg_image_overlap_weight: float = 0.55,
) -> float:
    pair = {int(label_a), int(label_b)}
    if pair == {2, 5}:
        return 0.0
    if _is_svg_text_pair(label_a, label_b):
        return float(max(svg_text_overlap_weight, 0.0))
    if _is_svg_image_pair(label_a, label_b):
        return float(max(svg_image_overlap_weight, 0.0))
    return 1.0


def weighted_pairwise_overlap_area(
    boxes: np.ndarray,
    labels: np.ndarray,
    svg_text_overlap_weight: float = 0.35,
    svg_image_overlap_weight: float = 0.55,
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if weight <= 0.0:
                continue
            min_area = min(_box_area_xywh(boxes[i]), _box_area_xywh(boxes[j]))
            allowance = overlap_allowance_ratio(
                li,
                lj,
                svg_text_allowance_ratio=svg_text_allowance_ratio,
                svg_image_allowance_ratio=svg_image_allowance_ratio,
            ) * min_area
            ov += weight * max(0.0, inter - allowance)
    return float(ov)
    ...
    for _ in range(int(steps)):
        ...
        for i in range(len(boxes)):
            for j in range(i + 1, len(boxes)):
                ...
                current_inter = intersection_area(boxes[i], boxes[j])
                min_area = min(_box_area_xywh(boxes[i]), _box_area_xywh(boxes[j]))
                allowance = overlap_allowance_ratio(
                    li,
                    lj,
                ...
                if li == 5 or lj == 5:
                    ...
                    gap = gap_text
                elif _is_svg_text_pair(li, lj) or _is_svg_image_pair(li, lj):
                    if current_inter <= allowance:
                        continue
                    gap = 0.0
                else:
                    gap = gap_other
                ...
                push_scale = 1.0
                if _is_svg_text_pair(li, lj):
                    push_scale = float(max(svg_text_overlap_weight, 0.0))
                elif _is_svg_image_pair(li, lj):
                    push_scale = float(max(svg_image_overlap_weight, 0.0))
                push_scale = float(np.clip(push_scale, 0.15, 1.0))

                dx = float(boxes[i, 0] - boxes[j, 0])
                dy = float(boxes[i, 1] - boxes[j, 1])
                if abs(dx) >= abs(dy):
                    push = (ox / 2.0 + 1e-3) * push_scale
                    sgn = 1.0 if dx >= 0.0 else -1.0
                    boxes[i, 0] += sgn * push
                    boxes[j, 0] -= sgn * push
                else:
                    push = (oy / 2.0 + 1e-3) * push_scale
...
def build_auto_svg_inputs(default_svg_aspects=None):
    aspects = []
    for x in (default_svg_aspects or []):
        try:
            x = float(x)
        except Exception:
            continue
        if x > 0:
            aspects.append(x)
    if len(aspects) == 0:
        aspects = [1.0, 1.4, 0.75]
    infos = []
    for i, aspect in enumerate(aspects):
        infos.append({
            'path': f'__auto_svg_{i}__',
            'aspect': float(aspect),
        })
    return infos
...
def enforce_image_aspect_ratios(
    ...
    """Keep every label=2 image box at the original image aspect ratio exactly.

    Important: use the same exact-aspect fitting logic as SVG, so later size bounds
    or canvas fitting do not independently clip width/height and break the aspect ratio.
    """
...
def score_layout(
    ...
    ov = weighted_pairwise_overlap_area(
        boxes,
        labels,
        svg_text_overlap_weight=svg_text_overlap_weight,
        svg_image_overlap_weight=svg_image_overlap_weight,
    ...
    meta = {
        ...
        "svg_text_overlap_weight": float(svg_text_overlap_weight),
        "svg_image_overlap_weight": float(svg_image_overlap_weight),
        "svg_text_allowance_ratio": float(svg_text_allowance_ratio),
        "svg_image_allowance_ratio": float(svg_image_allowance_ratio),
    }
...
def validate_custom_labels(labels, max_elems, add_face, image_infos, svg_infos):
    ...
    if svg_infos and any(not str(x.get("path", "")).startswith("__auto_svg_") for x in svg_infos) and 0 not in labels:
...
def sample_best_of_k(
    ...
    for t in range(mode_cfg["effective_k"]):
        ...
        # Final strict pass: restore exact image aspect ratios after all repair steps.
        cand_boxes = enforce_image_aspect_ratios(
            cand_boxes,
            labels=cand_labels,
            image_infos=image_infos,
            preserve_image_aspect=preserve_image_aspect,
...
def main():
    ...
    parser.add_argument("--auto_svg_aspect", type=int, default=1,
                        help="1=若未提供 --svg_paths，則自動建立 SVG 資產比例，不需要使用者丟 SVG")
    parser.add_argument("--default_svg_aspects", type=float, nargs="*", default=[1.0, 1.4, 0.75],
                        help="auto SVG 模式下使用的預設 W/H 比例，可循環套用到 label=0")
    ...
    parser.add_argument("--preview_w", type=int, default=120)
    parser.add_argument("--preview_h", type=int, default=80)
    ...
    parser.add_argument("--svg_text_overlap_weight", type=float, default=0.35,
                        help="SVG 與文字重疊的懲罰權重；越小越允許重疊")
    parser.add_argument("--svg_image_overlap_weight", type=float, default=0.55,
                        help="SVG 與圖片重疊的懲罰權重；越小越允許重疊")
    parser.add_argument("--svg_text_allowance_ratio", type=float, default=0.18,
                        help="SVG 與文字允許的合理重疊比例，相對於兩者較小 box 的面積")
    parser.add_argument("--svg_image_allowance_ratio", type=float, default=0.12,
                        help="SVG 與圖片允許的合理重疊比例，相對於兩者較小 box 的面積")
    ...
    svg_infos = []
    has_svg_label = (args.labels is not None and 0 in set(int(x) for x in args.labels)) or (args.labels is None)
    if svg_paths:
    ...
    elif int(args.auto_svg_aspect) == 1 and has_svg_label:
        svg_infos = build_auto_svg_inputs(args.default_svg_aspects)
        print("[SVGS] auto svg mode enabled; no user SVG file is required.")
        print("[SVGS] num auto svg aspect presets:", len(svg_infos))
        for idx, info in enumerate(svg_infos):
            print(f"[SVGS] #{idx}: path={info['path']}")
            print(f"[SVGS] #{idx}: aspect={info['aspect']}")
    ...
    for i in tqdm(range(args.n), ncols=100):
        ...
        boxes, labels, meta = sample_best_of_k(
            ...
            svg_text_overlap_weight=args.svg_text_overlap_weight,
            svg_image_overlap_weight=args.svg_image_overlap_weight,
            svg_text_allowance_ratio=args.svg_text_allowance_ratio,
            svg_image_allowance_ratio=args.svg_image_allowance_ratio,
        )
    ...
    print("auto_svg_aspect:", args.auto_svg_aspect)
    print("default_svg_aspects:", args.default_svg_aspects)
    ...
    print("svg_text_overlap_weight:", args.svg_text_overlap_weight)
    print("svg_image_overlap_weight:", args.svg_image_overlap_weight)
    print("svg_text_allowance_ratio:", args.svg_text_allowance_ratio)
    print("svg_image_allowance_ratio:", args.svg_image_allowance_ratio)
    if args.labels is not None:
