...
def content_region_loss(
    ...
    """
    把所有前景元素包成一個外接矩形，然後要求這個「整體內容區」落在某個 template 指定的位置和大小裡。
    只看像是 text、image、svg 這些排版主體
    把這些有效元素從 (cx, cy, w, h) 轉成 (x1, y1, x2, y2) (cx,cy)是理想中心點
    x1.min() 當整體內容區左界
    y1.min() 當上界
    x2.max() 當右界
    y2.max() 當下界 
    ...
    cfg = {
        "center_vertical": {"xmin": 0.14, "xmax": 0.86, "ymin": 0.10, "ymax": 0.90, "cx": 0.50, "cy": 0.54, "max_w": 0.62, "max_h": 0.78},
        "right_column":    {"xmin": 0.48, "xmax": 0.90, "ymin": 0.12, "ymax": 0.90, "cx": 0.70, "cy": 0.54, "max_w": 0.38, "max_h": 0.78},
        "top_hero":        {"xmin": 0.14, "xmax": 0.88, "ymin": 0.26, "ymax": 0.92, "cx": 0.52, "cy": 0.62, "max_w": 0.70, "max_h": 0.60},
        "bottom_caption":  {"xmin": 0.12, "xmax": 0.88, "ymin": 0.08, "ymax": 0.78, "cx": 0.50, "cy": 0.40, "max_w": 0.74, "max_h": 0.62},
        "hybrid":          {"xmin": 0.28, "xmax": 0.92, "ymin": 0.20, "ymax": 0.94, "cx": 0.62, "cy": 0.58, "max_w": 0.62, "max_h": 0.68},
    }[template]
    ...
    for b in range(bbox_fake.size(0)):
        ...
        loss_b = torch.tensor(0.0, device=device)
        # keep content union inside target region
        loss_b += torch.relu(cfg["xmin"] - x1)
        loss_b += torch.relu(x2 - cfg["xmax"])
        loss_b += torch.relu(cfg["ymin"] - y1)
        loss_b += torch.relu(y2 - cfg["ymax"])

        # prevent content union from growing too large and eating all whitespace
        loss_b += compact_weight * torch.relu(cw - cfg["max_w"])
        loss_b += compact_weight * torch.relu(ch - cfg["max_h"])

        # softly bias content centroid toward the template region center
        loss_b += center_weight * ((ccx - cfg["cx"]) ** 2 + (ccy - cfg["cy"]) ** 2)

        # template-specific extra bias to make whitespace shape more intentional
        if template == "hybrid":
            loss_b += 1.5 * torch.relu(0.18 - x1)
            loss_b += 1.5 * torch.relu(0.18 - y1)
        elif template == "top_hero":
            loss_b += 1.2 * torch.relu(0.18 - y1)
        elif template == "right_column":
            loss_b += 1.2 * torch.relu(0.18 - x1)
...
def content_union_stats(
    ...
    """Diagnostics for the union box of all foreground elements."""
    device = bbox_fake.device
    union_w, union_h = [], []
    union_x1, union_y1 = [], []
    for b in range(bbox_fake.size(0)):
        ...
        if valid.sum() == 0:
            continue
        xyxy = xywh_to_xyxy(bbox_fake[b][valid])
        x1 = xyxy[:, 0].min(); y1 = xyxy[:, 1].min(); x2 = xyxy[:, 2].max(); y2 = xyxy[:, 3].max()
    ...
    if len(union_w) == 0:
        ...
        return z, z, z, z
    return torch.stack(union_w).mean(), torch.stack(union_h).mean(), torch.stack(union_x1).mean(), torch.stack(union_y1).mean()
        ...
        for data in train_dataloader:
            ...
            if iteration % 100 == 0:
                ...
                with torch.no_grad():
                    ...
                    union_w_mean, union_h_mean, union_x1_mean, union_y1_mean = content_union_stats(
