"""用 train_fixed_v6_0.6pkl_v58 改 v59
1. 只改 frame_vertical 專用 loss，做真正的四周留白
2. 不動其他模板的留白邏輯
"""
import os
import csv
        ...
        for data in train_dataloader:
            ...
            if iteration % 100 == 0:
                ...
                print(f'[{iteration}] Loss_D: {loss_D.item():.4f} Loss_G: {loss_G.item():.4f} '
                      f'STYLE:{curr_lambda_style:.3f} SPACE:{curr_lambda_space:.3f} FRAME:{loss_frame_balance.item():.3f} OBAND:{loss_outer_band.item():.3f} FSHAPE:{loss_frame_shape.item():.3f} REGION:{loss_region.item():.3f} FG:{loss_fg_area.item():.3f} TXTW:{(curr_lambda_text_width * loss_text_width).item():.3f} '
