    ...
    for epoch in range(max_epoch):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            if iteration % args.vis_every == 0:
                ...
                # 2) 給 visualization 的 valid_mask：True=要畫；排除 3/4
                #    注意：此處假設 save_image 的 mask 語意是 True=valid(要畫)。
                #    若你的 save_image 使用 True=padding(不畫)，請改回：
                #      vis_mask = gen_padding_mask | hide_mask
                ...
                if fixed_bbox_real is not None:
                    ...
                    save_image(
                        fixed_bbox_real, fixed_label, vis_mask,
                ...
                with torch.no_grad():
                    ...
                    # 注意：這裡用 gen_padding_mask（不要因為想隱藏就改生成）
                    bbox_vis = netG(fixed_z, fixed_label, gen_padding_mask)

                    # 存圖才用 vis_mask 隱藏 3/4
                    save_image(
                        bbox_vis, fixed_label, vis_mask,
