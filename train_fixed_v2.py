    ...
    for epoch in range(max_epoch):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            if iteration % args.vis_every == 0:
                # --- 強力過濾邏輯：白名單策略 ---
                # 我們只保留：1 (Text), 2 (Image), 5 (Face)
                # 其餘所有標籤 (0, 3, 4 等) 通通設為隱藏
                keep_ids = [0, 1, 2, 5]
                
                # 建立一個基礎為 True (全部隱藏) 的遮罩
                # 然後把 1, 2, 5 的位置設為 False (不隱藏)
                ignore_mask = torch.ones_like(fixed_label, dtype=torch.bool)
                for ki in keep_ids:
                    ignore_mask &= (fixed_label != ki)
                
                # 最終遮罩 = 原始 Padding 遮罩 OR 我們要忽略的標籤遮罩
                vis_mask = fixed_mask | ignore_mask

                # 調試訊息：如果你在終端機看到這個，代表過濾邏輯有在跑
                if iteration == 0:
                    print(f"DEBUG: 正在生成圖片，已隱藏除 {keep_ids} 以外的所有標籤。")

                # --- 1. 生成 Real Sample ---
                if fixed_bbox_real is not None:
                    out_path_real = out_dir / 'real_samples.png'
                    save_image(fixed_bbox_real, fixed_label, vis_mask, 
                ...
                with torch.no_grad():
                    ...
                    save_image(bbox_vis, fixed_label, vis_mask, 
