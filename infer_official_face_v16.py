...
def enforce_image_aspect_ratio(
    ...
    """Keep label=2 image boxes close to the user's original image aspect ratio.

    LayoutGAN predicts only rectangular boxes, so without this pass a horizontal
    uploaded image can become a vertical generated image region. This pass keeps
    the generated image area approximately unchanged, but adjusts width/height so
    w / h ~= original_image_width / original_image_height.
    """
    if (not preserve_image_aspect) or image_aspect is None or image_aspect <= 0:
        return boxes
    ...
    aspect = float(np.clip(float(image_aspect), 0.25, 4.0))
...
def render_user_image_layout(
    ...
    """Render a poster preview using the real uploaded image for label=2.

    label meaning in this project:
    - 2: image region, filled by the user's uploaded image
    - 5: face region, not drawn by default because it is already inside image
    - 1: text placeholder
    - 0: SVG/decorative placeholder

    image_fit_mode:
    - cover: recommended for poster design. Fill the image box without distortion;
      overflow is cropped. If a face is detected, crop around the face.
    - contain: show the full image without distortion; blank padding may appear.
    """
    if Image is None:
        raise ImportError("Pillow is required. Please run: pip install pillow")
    if not user_image_path:
        raise ValueError("user_image_path is empty.")

    canvas_w, canvas_h = int(out_size[0]), int(out_size[1])
    canvas = Image.new("RGB", (canvas_w, canvas_h), bg_color)
    user_img = Image.open(str(user_image_path)).convert("RGB")
    face_focus = _face_focus_from_detected_faces(detected_face_boxes)

    # Draw image first, then text/svg placeholders above it.
    image_idxs = np.where(labels == 2)[0].tolist()
    for idx in image_idxs:
        x1, y1, x2, y2 = _box_to_pixel_xyxy(boxes[idx], canvas_w, canvas_h)
        bw, bh = max(1, x2 - x1), max(1, y2 - y1)
        if image_fit_mode == "contain":
            patch = resize_contain_no_distort(user_img, bw, bh, bg_color=bg_color)
        elif image_fit_mode == "cover":
            patch = resize_cover_face_aware(user_img, bw, bh, face_focus_norm=face_focus)
        else:
    ...
    # Use simple placeholders so the preview can be inspected visually without
    # requiring actual typography/SVG assets.
    try:
    ...
    for box, lb in zip(boxes, labels):
        ...
        if lb == 1:
            # text placeholder: translucent green block
            draw.rectangle([x1, y1, x2, y2], fill=(70, 180, 90, 130), outline=(20, 130, 50, 230), width=2)
        elif lb == 0:
            # svg placeholder: translucent blue block
...
def append_face_if_needed(
    ...
    """Append only real YOLO-detected faces.

    Required behavior:
    - The generated non-face elements must follow the user-specified labels exactly.
    - If YOLO detects faces, append one label=5 box per detected face.
    - If YOLO detects no faces, do NOT synthesize or fallback-add any face.

    Therefore ADD_FACE=1 means "allow detected faces to be appended", not
    "always force one face".
    """
    if not add_face:
    ...
    if not detected_face_boxes:
        # Important: no detected face means no label=5 is added.
        return boxes, labels, img_idx

    face_boxes = [
        map_detected_face_to_image_box(boxes[img_idx], face_norm)
        for face_norm in detected_face_boxes
    ]
...
def main():
    ...
    parser.add_argument("--image_path", type=str, default="",
                        help="使用者上傳圖片路徑；若提供，會先用 YOLOv8n-face 偵測實際人臉位置")
    ...
    detected_face_boxes = []
    if args.image_path:
        image_aspect = get_user_image_aspect(args.image_path)
        print("[IMAGE] original aspect W/H:", image_aspect)
        detected_face_boxes = detect_faces_yolov8_norm(
            image_path=args.image_path,
            model_path=args.face_model,
            conf=args.face_conf,
            max_faces=args.max_detected_faces,
        )
        print("[YOLO] image_path:", args.image_path)
        print("[YOLO] detected faces:", len(detected_face_boxes))
        print("[YOLO] normalized face boxes:", detected_face_boxes)
        if len(detected_face_boxes) == 0:
            print("[YOLO][WARN] No face detected. No face element will be added.")
            detected_face_boxes = []
    ...
    for i in tqdm(range(args.n), ncols=100):
        if args.labels is not None:
            validate_custom_labels(
                ...
                args.max_elems,
                bool(args.add_face),
                detected_face_count=len(detected_face_boxes) if bool(args.add_face) else 0,
            )
        ...
        if bool(args.render_user_image) and args.image_path:
    ...
    if args.labels is not None:
        detected_count = len(detected_face_boxes) if bool(args.add_face) else 0
