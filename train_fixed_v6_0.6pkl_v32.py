"""用 train_fixed_v6_0.6pkl_v30 改 v31
    1. 修改留白風格
"""
import os
import argparse
...
def main():
    ...
    for epoch in range(10000):
        for data in train_dataloader:
            ...
            cx = bbox_fake[:, :, 0]
            cy = bbox_fake[:, :, 1]
            x1 = (cx - bbox_fake[:, :, 2] / 2).clamp(0, 1)
            x2 = (cx + bbox_fake[:, :, 2] / 2).clamp(0, 1)
            y1 = (cy - bbox_fake[:, :, 3] / 2).clamp(0, 1)
            y2 = (cy + bbox_fake[:, :, 3] / 2).clamp(0, 1)

            # 隨機選擇一種風格 (模擬你提供的四張參考圖指標)
            style_idx = np.random.randint(0, 4) 
            loss_style = torch.tensor(0.0, device=device)

            if style_idx == 0: # Style A: 【左右/側邊】(對應 左右.jpg) 
                # 強迫右邊界靠近 0.85，左側留出大面積留白 (S_side 邏輯)
                loss_style = torch.abs(x2[mask] - 0.85).mean()
            
            elif style_idx == 1: # Style B: 【置中平衡】(對應 中.jpg)
                # 強迫中心對齊 0.5，四周均勻留白 (S_frame 邏輯)
                loss_style = torch.abs(cx[mask] - 0.5).mean()
            
            elif style_idx == 2: # Style C: 【上下排版】(對應 上下.jpg)
                # 圖片在上 (y1=0.2)，文字在下 (y2=0.8) (S_tb 邏輯)
                img_m, txt_m = (label == IMG_ID) & mask, (label == TEXT_ID) & mask
                if img_m.any() and txt_m.any():
                    loss_style = torch.abs(y1[img_m] - 0.2).mean() + torch.abs(y2[txt_m] - 0.8).mean()
            
            elif style_idx == 3: # Style D: 【角落/底部】(對應 角落.jpg)
                # 元件壓低到 y2=0.95，上方大面積留白
                loss_style = torch.abs(y2[mask] - 0.95).mean()

            loss_G += (loss_style * 80.0) 

            # [補強] 元件間的對齊一致性 (模擬指標中的 std_m 懲罰)
            if mask.any():
                # 強迫 X 座標標準差降低，確保文字和圖片站成整齊的一排
                loss_consistency = x1[mask].std() + cx[mask].std()
                loss_G += (loss_consistency * 50.0)
            ...
            loss_G += (curr_lambda_ws * whitespace_style_loss(bbox_fake, padding_mask, args.wr_min)) + \
                      (curr_lambda_ov * pairwise_overlap_loss(bbox_fake, label, padding_mask)) + \
                      face_coverage_loss(bbox_fake, label, padding_mask, args.lambda_face) + \
                      element_size_loss(bbox_fake, label, padding_mask, [IMG_ID], 0.35) + \
                      containment_loss(bbox_fake, label, padding_mask, lambda_cont=250.0) + \
                      alignment_loss(bbox_fake, padding_mask) + \
                      face_anchor_loss(bbox_fake, label, padding_mask, face_pos, face_mask, lambda_anchor=30.0) + \
                      (loss_aspect * 50.0) + (loss_min_size * 100.0)
