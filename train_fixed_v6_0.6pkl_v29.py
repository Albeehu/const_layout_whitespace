"""用 train_fixed_v6_0.6pkl_v28 改 v29
...
def main():
    ...
    for epoch in range(10000):
        for data in train_dataloader:
            ...
            # 取得左邊界座標 x1
            x1_fake = (bbox_fake[:, :, 0] - bbox_fake[:, :, 2] / 2).clamp(0, 1)
            # 懲罰不靠左 (0.1) 也不置中 (0.5) 的排版行為
            loss_grid = torch.min(torch.abs(x1_fake - 0.1), torch.abs(x1_fake - 0.5)).mean()
            loss_G += (loss_grid * 40.0)

            text_mask = (label == TEXT_ID) & valid_mask
            if text_mask.any():
                text_w = bbox_fake[text_mask][:, 2]
                # 強迫文字寬度至少要有 0.25 (畫布的 1/4)
            ...
            loss_G += (curr_lambda_ws * whitespace_style_loss(bbox_fake, padding_mask, args.wr_min)) + \
                      ...
                      containment_loss(bbox_fake, label, padding_mask, lambda_cont=200.0) + \
